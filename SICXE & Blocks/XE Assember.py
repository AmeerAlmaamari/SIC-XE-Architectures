...
class Entry:
    def __init__(self, string, token, attribute, block=0):
        ...
        self.block = block # block initilization
...
def lexan():
    ...
    if fileContent[bufferindex].isdigit():
        ...
        if (fileContent[bufferindex].upper() == 'C') and (fileContent[bufferindex+1] == '\''):
            ...
            if p == -1:
                if startLine == True:
                    p=insert(fileContent[bufferindex].upper(),'ID',locctr[block], block) # should we deal with case-sensitive?
                else:
                    p=insert(fileContent[bufferindex].upper(),'ID',-1, -1) #forward reference
            else:
                if (symtable[p].attrib == -1) and (startLine == True):
                    symtable[p].attrib = locctr[block]
...
def checkRelAddressRange(PCrelAddress):
    ...
    baseRelAddress = baseValue - locctr[block]
    if 2047 >= PCrelAddress and PCrelAddress >= -2048:
        ...
        error('Relative addressing out of range (B and P) in line:', lineno)
        return None
...
def rest4():
    ...
    if lookahead == ",": # , REG
        match(',')
        if pass1or2 == 2:
            inst += symtable[tokenval].attrib # REG2 bits
        match('REG')
...
def rest6(plusPrefix, hashOrAt):   # For # and @
    ...
    if lookahead == 'ID':  
        if pass1or2 == 2:
            PCRelAddress = symtable[tokenval].attrib - locctr[block]  # Get the PC rel address
        ...
        if pass1or2 == 2:
            if hashOrAt == '#':  # Immedate
                ...
                if plusPrefix:     # +F3 format, False if NO PLUS
                    ...
                    PCRelAddress = symtable[tokenval].attrib - locctr[block]  # Get the PC rel address
                    baseRelAddress = baseValue - symtable[tokenval].attrib   # Get the Base rel address
                    PCorBase = checkRelAddressRange(PCRelAddress)
                    if PCorBase == 'PC':
...
def rest5(prevStmtIndex, plusPrefix=False):  # For F3 and +F3
    ...
    if lookahead == 'ID':
        if startLine == False: # not taking into account the next line IDs
            if pass1or2 == 2:
                if plusPrefix:     # +F3 format, False if NO PLUS
                    inst += symtable[tokenval].attrib # Absolute addressing
                    inst += Nbitset << 24
                    inst += Ibitset << 24
                    inst += Ebit4set
                else:               # F3 Format
...
def stmt():
    ...
    if lookahead == 'f1':
        ...
        if pass1or2 == 2:
            inst = symtable[tokenval].attrib << 8 # shift the instruction by 16 bits, to take the instruction from right side to left side to work
        locctr[block] += 2
        match('f2')
        startLine = True
        if pass1or2 == 2:
            inst += symtable[tokenval].attrib << 4 # REG1 bits
        match('REG')
        rest4()
        ...
        if pass1or2 == 2:
            inst = symtable[tokenval].attrib << 16 # shift the instruction by 16 bits, to take the instruction from right side to left side to work
        locctr[block] += 3
        match('f3')
        rest5(prevStmtIndex)
        ...
        match('+')
        if pass1or2 == 2:
            inst = symtable[tokenval].attrib << 24 # Take the instruction to the "opcode" part
        locctr[block] += 4
        match('f3')
...
def rest2():
    ...
    if lookahead == 'STRING':
        size = int(len(symtable[tokenval].attrib) / 2) # each letter is 8 bits, we need a string hex value, we divide by 2 to make it into half byte for each letter
        locctr[block] += size
        if pass1or2 == 2:
            if objectCode:
                print('T {:06x} {:02x}'.format(locctr[block]-size, size)+' '+symtable[tokenval].attrib)
            else:
...
def body():
    ...
    if lookahead == 'ID': #ID
        ...
        match('BASE')
        if pass1or2 == 2:
            baseValue = symtable[tokenval].attrib
        match('ID')
        body()
    ...
    if pass1or2 == 1:
        ...
        for symbol in symtable:
            if symbol.token == 'ID' and symbol.block == 1:
                symbol.attrib += sizeDefault
            elif symbol.token == 'ID' and symbol.block == 2:
                symbol.attrib += sizeDefault + sizeCDATA # = sizeCBLCK
